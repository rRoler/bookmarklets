	...
	return {
		...
		renderChunk: (code) => {
			...
			options.config.bookmarklets.forEach((bookmarklet) => {
				...
				if (!tableOfContentsForWebsite[bookmarklet.website.name])
					tableOfContentsForWebsite[bookmarklet.website.name] = '';
				if (!bookmarkletsForWebsite[bookmarklet.website.name])
					bookmarkletsForWebsite[bookmarklet.website.name] = '';

				tableOfContentsForWebsite[
					bookmarklet.website.name
				] += `	- [${headerName}](#${headerId})\n`;
			...
			for (const website in bookmarkletsForWebsite) {
				tableOfContents +=
					`- [${website}](#${getHeaderId(website)})\n` +
					tableOfContentsForWebsite[website];
				bookmarkletsInfo +=
					`# ${website}\n` + bookmarkletsForWebsite[website] + '***\n';
			}
